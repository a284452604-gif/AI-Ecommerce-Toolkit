...
### 变更（设计原则）
- **所有优化风格强制基于平台数据**，禁止 AI 凭经验编造通用高频词
  - 重写四种风格的 `system_prompt`，统一追加「强制数据约束」：标题与关键词只能由用户提供的平台搜索词榜单真实关键词（搜索词/核心词/趋势词/修饰词）组合而成
  - 在 `optimize()` 核心层强制要求 `market_data` 或 `image_paths`，缺失时直接返回失败并提示先上传/填写平台数据
  - 页面「平台市场数据」由可选改为**必填**，未提供数据时点「开始优化」会被拦截并给出指引

### 新增
- **截图一键识别并填充表格**
  - 新增 `market_data_extractor.py`：OCR 截图文字 → 调用 AI 整理为 8 列搜索词榜单 JSON 数组 → 校验清洗
  - 标题优化页新增「识别截图并填充表格」按钮（异步 QThread），自动把截图里的榜单数据填入可编辑表格，供用户核对后再优化
  - 支持 vision 模型直接传图；非 vision 模型自动走 OCR 文字提取
